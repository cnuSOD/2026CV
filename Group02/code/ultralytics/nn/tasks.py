...
from ultralytics.nn.modules.PhysicalFeatureGuidance import (
    PhysicalFeatureGuidance as PFG,
    PhysicalBranch as PB,
)

from ultralytics.nn.modules.FEM import FEM
from ultralytics.nn.modules.CKConv import CKConv
from ultralytics.nn.modules.SPD import SPD
from ultralytics.nn.modules.efficient_multiscale_attention import EMA
from ultralytics.nn.modules.SCAM import SCAM

from ultralytics.nn.modules.PhysicalOriginalFeatureFusion import (
    PhysicalOriginalFeatureFusion as POFF,
    PhysicalOriginalGTFeatureFusion as POGTFF,
)
...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        # 加入物理特征
        # x0 = x
        # physical_p1, physical_p2, physical_p3 = self.physical_branch(x0)
        ...
        for m in self.model:
            ...
            # 加入物理特征层，2->P2，4->P3
            # if m.i == 2:
            #     x = self.p1_fusion(x, physical_p1)
                # x, m_pre_p1 = self.p1_fusion(x, physical_p1)
                # aux_outputs["m_pre_p1"] = m_pre_p1

            # if m.i == 4:
            #     x = self.p2_fusion(x, physical_p2)
                # x, m_pre_p2 = self.p2_fusion(x, physical_p2)
                # aux_outputs["m_pre_p2"] = m_pre_p2

            # if m.i == 6:
            #     x = self.p3_fusion(x, physical_p3)
                # x, m_pre_p3 = self.p3_fusion(x, physical_p3)
                # aux_outputs["m_pre_p3"] = m_pre_p3
        ...
        # 加入辅助输出到返回值中，方便计算PGDP损失
        # if isinstance(x, dict):
        #     x.update(aux_outputs)
        #     # if "m_pre" not in x:
        #     #     x["m_pre"] = aux_outputs.get("m_pre_p3", aux_outputs.get("m_pre_p2"))
        #     x["m_pre_dict"] = {
        #         "p1": aux_outputs.get("m_pre_p1"),
        #         "p2": aux_outputs.get("m_pre_p2"),
        #         "p3": aux_outputs.get("m_pre_p3"),
        #     }
        #     x["m_pre_dict"] = {k: v for k, v in x["m_pre_dict"].items() if v is not None}

        #     x["m_pre_list"] = list(x["m_pre_dict"].values())

        #     if "m_pre" not in x and len(x["m_pre_list"]) > 0:
        #         x["m_pre"] = x["m_pre_list"][-1]

        # elif isinstance(x, tuple) and len(x) == 2 and isinstance(x[1], dict):
        #     x[1].update(aux_outputs)
        #     # if "m_pre" not in x[1]:
        #     #     x[1]["m_pre"] = aux_outputs.get("m_pre_p3", aux_outputs.get("m_pre_p2"))
        #     x[1]["m_pre_dict"] = {
        #         "p1": aux_outputs.get("m_pre_p1"),
        #         "p2": aux_outputs.get("m_pre_p2"),
        #         "p3": aux_outputs.get("m_pre_p3"),
        #     }
        #     x[1]["m_pre_dict"] = {k: v for k, v in x[1]["m_pre_dict"].items() if v is not None}

        #     x[1]["m_pre_list"] = list(x[1]["m_pre_dict"].values())

        #     if "m_pre" not in x[1] and len(x[1]["m_pre_list"]) > 0:
        #         x[1]["m_pre"] = x[1]["m_pre_list"][-1]
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        # 加入物理特征引导模块
        # self.physical_branch = PB(cutoff_ratio=0.1)
        # self.p1_fusion = POFF(visual_dim=64, physical_dim=32)
        # self.p2_fusion = POFF(visual_dim=128, physical_dim=64)
        # self.p3_fusion = POFF(visual_dim=128, physical_dim=128)
        # self.p1_fusion = POGTFF(visual_dim=64, physical_dim=32)
        # self.p2_fusion = POGTFF(visual_dim=128, physical_dim=64)
        # self.p3_fusion = POGTFF(visual_dim=128, physical_dim=128)
    ...
    def init_criterion(self):
        ...
        # # 加入POGTFF损失函数
        # return v8DetectionPGDPLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            c1, c2 = ch[f], args[0]
            if m is EMA:
                # EMA is an attention block that preserves channel count.
                c2 = c1
                args = [c1, *args[1:]]
            ...
            if len(args) >= 2:
                ...
                args = [c1, c2, block_size, k]
            else:
                block_size = args[0] if args else 2
                c2 = ch[f] * block_size**2
                args = [None, None, block_size]

        elif m is POFF:
            c1_visual = ch[f[0]]
            c1_physical = ch[f[1]]
            
            args = [c1_visual, c1_physical]
            c2 = c1_visual
        else:
            c2 = ch[f]
