...
# 加入POGTFF的损失
class v8DetectionPGDPLoss(v8DetectionLoss):
    def __init__(self, model, tal_topk=10, tal_topk2=None, pgdp_weight=0.2):
        super().__init__(model, tal_topk, tal_topk2)
        self.pgdp_weight = pgdp_weight
    ...
    def weighted_mse_loss(
        ...
        if pred.shape != target.shape:
            raise ValueError(
                f"`pred` and `target` must share the same shape, got "
                f"{tuple(pred.shape)} and {tuple(target.shape)}."
            )

        positive_mask = target > thr
        weights = torch.where(
            positive_mask,
    ...
    def build_single_image_targets(self, batch, i):
        mask = batch["batch_idx"].view(-1) == i
        cls_i = batch["cls"][mask].view(-1, 1).to(self.device)
        box_i = batch["bboxes"][mask].to(self.device)   # xywh, normalized
        if cls_i.numel() == 0:
            return torch.empty((0, 5), dtype=torch.float32, device=self.device)
        return torch.cat([cls_i.float(), box_i], dim=1)
    ...
    def _resolve_device(
        ...
        """Return a valid device for new tensors."""
    ...
    def _prepare_targets(
        ...
        """
        Validate and move YOLO-format targets to the requested device.
        ...
        if targets.ndim != 2 or targets.shape[-1] != 5:
            raise ValueError(
                f"`targets` must have shape [N, 5], but got {tuple(targets.shape)}."
            )

        target_device = self._resolve_device(device, targets)
        if targets.numel() == 0:
            return torch.empty((0, 5), dtype=torch.float32, device=target_device)

        return targets.to(device=target_device, dtype=torch.float32)
    ...
    def _filter_valid_targets(
        ...
        """
        Remove invalid or degenerate boxes before Gaussian generation.

        Filtering strategy:
        1. Keep boxes whose normalized width/height are positive.
        2. Keep boxes whose centers are finite.
        3. Reject extremely small boxes after projection to the output scale.
        This avoids unstable supervision caused by abnormal labels.
        """
        if targets.numel() == 0:
            return targets
        ...
        finite_mask = torch.isfinite(targets).all(dim=1)
        center_mask = (cx >= 0.0) & (cx <= 1.0) & (cy >= 0.0) & (cy <= 1.0)
        positive_mask = (w > 0.0) & (h > 0.0)

        w_out = w * float(out_w)
        h_out = h * float(out_h)
        min_valid_size = max(eps, 0.5)
        min_size_mask = (w_out >= min_valid_size) & (h_out >= min_valid_size)

        valid_mask = finite_mask & center_mask & positive_mask & min_size_mask
        return targets[valid_mask]
    
    def _size_aware_alpha_from_size(self, obj_size: torch.Tensor) -> torch.Tensor:
        """
        Select alpha by projected object size.

        Smaller alpha leads to a smaller sigma because sigma = size / alpha.
        A smaller sigma makes the Gaussian sharper and more concentrated around
        the center, which increases the positional emphasis for tiny objects.
        """
        alpha = torch.full_like(obj_size, 10.0)
        alpha = torch.where(obj_size <= 32.0, torch.full_like(alpha, 8.0), alpha)
        alpha = torch.where(obj_size < 16.0, torch.full_like(alpha, 6.0), alpha)
        alpha = torch.where(obj_size < 8.0, torch.full_like(alpha, 4.0), alpha)
        return alpha
    
    def yolo_bbox_to_gaussian_map(
        self,
        targets: torch.Tensor,
        out_h: int,
        out_w: int,
        ...
        use_size_aware_alpha: bool = True,
        eps: float = 1e-6,
        device: torch.device | None = None,
    ) -> torch.Tensor:
        """
        Convert YOLO-format boxes into a single-channel Gaussian supervision map.

        Args:
            targets:
                Tensor with shape [N, 5]. Each row is [class_id, cx, cy, w, h].
                The class id is kept for compatibility but is not used here.
            out_h, out_w:
                Spatial resolution of the target feature map.
            mode:
                "sum" to accumulate all Gaussian maps, or "max" to take the
                element-wise maximum across objects.
            normalized:
                If True, clamp the final map into [0, 1]. This is usually useful
                when the map is treated as a probability-like supervision target.
            use_size_aware_alpha:
                If True, use different alpha values for different object sizes so
                that tiny objects get sharper Gaussians.
            eps:
                Small positive constant used to avoid zero sigma.
            device:
                Optional device for the returned tensor.

        Returns:
            Tensor with shape [1, out_h, out_w].
        """
        if out_h <= 0 or out_w <= 0:
            raise ValueError(f"`out_h` and `out_w` must be positive, got {out_h}, {out_w}.")
        if mode not in {"sum", "max"}:
            raise ValueError(f"`mode` must be 'sum' or 'max', but got {mode!r}.")

        targets = self._prepare_targets(targets, device=device)
        target_device = targets.device

        gaussian_map = torch.zeros((1, out_h, out_w), dtype=torch.float32, device=target_device)
        targets = self._filter_valid_targets(targets, out_h=out_h, out_w=out_w, eps=eps)
        if targets.numel() == 0:
            return gaussian_map

        cx_out = targets[:, 1] * float(out_w)
        cy_out = targets[:, 2] * float(out_h)
        w_out = targets[:, 3] * float(out_w)
        h_out = targets[:, 4] * float(out_h)

        if use_size_aware_alpha:
            obj_size = torch.maximum(w_out, h_out)
            alpha = self._size_aware_alpha_from_size(obj_size)
        else:
            alpha = torch.full_like(w_out, 6.0)

        sigma_x = torch.clamp(w_out / alpha, min=eps)
        sigma_y = torch.clamp(h_out / alpha, min=eps)

        yy = torch.arange(out_h, dtype=torch.float32, device=target_device).view(1, out_h, 1)
        xx = torch.arange(out_w, dtype=torch.float32, device=target_device).view(1, 1, out_w)

        cx = cx_out.view(-1, 1, 1)
        cy = cy_out.view(-1, 1, 1)
        sx = sigma_x.view(-1, 1, 1)
        sy = sigma_y.view(-1, 1, 1)

        # Gaussian centered at the projected object center on the current scale.
        gaussian_per_object = torch.exp(
            -0.5 * (((xx - cx) / sx) ** 2 + ((yy - cy) / sy) ** 2)
        )

        if mode == "sum":
            fused = gaussian_per_object.sum(dim=0, keepdim=True)
        else:
            fused = gaussian_per_object.max(dim=0, keepdim=True).values
    ...
    def build_batch_mgt(self, batch, h, w):
        maps = []
        bs = int(batch["img"].shape[0])
        for i in range(bs):
            targets_i = self.build_single_image_targets(batch, i)
            m_gt_i = self.yolo_bbox_to_gaussian_map(
                targets=targets_i,
                out_h=h,
                out_w=w,
                mode="sum",
                normalized=True,
                use_size_aware_alpha=True,
                device=self.device,
            )
            maps.append(m_gt_i)
        return torch.stack(maps, dim=0)   # [B, 1, H, W]

    def _collect_pgdp_maps(self, preds):
        if "m_pre_dict" in preds and isinstance(preds["m_pre_dict"], dict):
            return [v for v in preds["m_pre_dict"].values() if v is not None]
    
        maps = []
        if "m_pre_p1" in preds:
            maps.append(preds["m_pre_p1"])
        if "m_pre_p2" in preds:
            maps.append(preds["m_pre_p2"])
        if "m_pre_p3" in preds:
            maps.append(preds["m_pre_p3"])
        if not maps and "m_pre" in preds:
            maps.append(preds["m_pre"])
        return maps

    def loss(self, preds, batch):
        batch_size = preds["boxes"].shape[0]
        det_loss, _ = self.get_assigned_targets_and_loss(preds, batch)[1:]

        loss = torch.zeros(4, device=self.device)  # box, cls, dfl, pgdp
        loss[0], loss[1], loss[2] = det_loss[0], det_loss[1], det_loss[2]

        m_pre_list = self._collect_pgdp_maps(preds)
        if m_pre_list:
            pgdp_loss = torch.zeros(1, device=self.device, dtype=loss.dtype)
            for m_pre in m_pre_list:
                _, _, h, w = m_pre.shape
                m_gt = self.build_batch_mgt(batch, h, w)
                pgdp_loss = pgdp_loss + self.weighted_mse_loss(m_pre, m_gt)
            loss[3] = (pgdp_loss / len(m_pre_list)).squeeze(0) * self.pgdp_weight
